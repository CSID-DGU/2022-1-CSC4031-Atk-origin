...
def get_stt(file_name):
    ...
    jit_path = os.path.join(os.getcwd(), "atk_STT.ptjit")
    model = torch.jit.load(jit_path)

    file_dir = str(Path.home()) + "/app/stt/files"
    print(file_dir)
    wavfile = glob(f"{file_dir}/{file_name}")
    print(wavfile)
    batches = split_into_batches(wavfile, batch_size=10)
    model_input = prepare_model_input(read_batch(batches[0]), device=device)

    output = model(model_input)
    for example in output:
        # print(decoder(example.cpu())) transcript
        transcript = decoder(example.cpu())
    return transcript
